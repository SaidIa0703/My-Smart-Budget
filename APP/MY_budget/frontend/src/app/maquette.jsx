...
const MySmartBudget = () => {
  ...
  const handleAddTransaction = async (e) => {
    ...
    if (!API_URL) {
      setMessage("❌ URL d'API manquante");
      return;
    }
  ...
  const getCategories = () => {
    ...
    transactions.forEach((t) => {
      ...
      if (amount < 0) {
        ...
        grouped[t.category] += Math.abs(amount);
      }
    });
    ...
    return Object.keys(grouped).map((cat, index) => ({
      name: cat,
      spent: Math.round(grouped[cat] * 100) / 100,
  ...
  const filteredTransactions = transactions.filter((t) => {
    ...
    const matchSearch = name.includes(query) || category.includes(query);
  ...
  const DashboardPage = () => (
    <div className="space-y-6 animate-fadeIn">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
          className={`bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-3xl p-6 text-white shadow-2xl transform transition-all duration-500 hover:scale-105 ${
        ...
        <div
          className={`bg-gradient-to-br from-emerald-400 to-cyan-500 rounded-3xl p-6 text-white shadow-2xl transform transition-all duration-500 hover:scale-105 delay-100 ${
            animateCards ? 'animate-slideInUp' : ''
          }`}
        >
        ...
        <div
          className={`bg-gradient-to-br from-rose-400 to-orange-400 rounded-3xl p-6 text-white shadow-2xl transform transition-all duration-500 hover:scale-105 delay-200 ${
            animateCards ? 'animate-slideInRight' : ''
          }`}
        >
          <div className="flex justify-between items-start">
            <div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={(props) => `${props.name}: €${props.spent}`}
                  outerRadius={100}
                  fill="#8884d8"
                  dataKey="spent"
                >
              ...
              <div className="flex items-center gap-4">
                <div className="text-3xl">
                  {transaction.amount > 0
                    ...
                    ? '🚗'
                    : transaction.category === 'Loisirs'
                    ? '🎮'
                    : transaction.category === 'Abonnements'
                    ? '📱'
                    : transaction.category === 'Santé'
                    ? '⚕️'
                    : '💸'}
                </div>
                <div>
          ...
          {(searchText || filterCategory) && (
            <div className="p-3 bg-indigo-100 border border-indigo-300 rounded-xl">
              <p className="text-indigo-700 font-medium">
                ...
                {filteredTransactions.length !== 1 ? 's' : ''} trouvé
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl">
              ...
              <p className="text-gray-600 mb-4">
                {searchText
                  ? `Aucune transaction ne correspond à "${searchText}"`
    ...
    const categoryBudgets = [
      {
        name: 'Alimentation',
        budget: 400,
        spent: categories.find((c) => c.name === 'Alimentation')?.spent || 0,
        color: '#FF6B6B',
        icon: '🛒',
      },
      {
        name: 'Transport',
        budget: 150,
        spent: categories.find((c) => c.name === 'Transport')?.spent || 0,
        color: '#4ECDC4',
        icon: '🚗',
      },
      {
        name: 'Loisirs',
        budget: 200,
        spent: categories.find((c) => c.name === 'Loisirs')?.spent || 0,
        color: '#45B7D1',
        icon: '🎮',
      },
    ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-purple-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col md:flex-row gap-6">
        <aside
          className={`md:w-64 bg-white rounded-3xl p-6 shadow-xl border border-gray-100 ${
