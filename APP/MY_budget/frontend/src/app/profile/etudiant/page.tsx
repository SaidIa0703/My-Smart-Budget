...
const Regle502030 = ({ stats, categoryData }: { stats: any; categoryData: any[] }) => {
  ...
  if (revenus === 0) return (
    <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
      <h3 className="text-xl font-bold mb-2 bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">📐 Règle 50/30/20</h3>
      <p className="text-violet-500 text-sm">Ajoute tes revenus pour voir ton analyse budgétaire.</p>
    </div>
  );

  const necessity = ['Alimentation', 'Transport', 'Études', 'Santé'];
  const wants = ['Loisirs', 'Abonnements', 'Restauration', 'Vêtements'];
  ...
  const bars = [
    { label: 'Nécessités', pct: Math.round((depNecessite / revenus) * 100), target: 50, color: '#6366f1' },
    { label: 'Envies', pct: Math.round((depEnvies / revenus) * 100), target: 30, color: '#8b5cf6' },
    { label: 'Épargne', pct: Math.round((epargne / revenus) * 100), target: 20, color: '#10b981' },
  ];

  return (
    <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
      <div className="flex justify-between items-center mb-5">
        <h3 className="text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">📐 Règle 50/30/20</h3>
        <span className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full font-medium border border-indigo-100">Conseil étudiant</span>
      </div>
          ...
          <div key={b.label}>
            <div className="flex justify-between items-center mb-1.5">
              ...
              <div className="flex items-center gap-2">
                <span className="font-bold text-slate-900 text-sm">{b.pct}%</span>
                <span className="text-xs text-violet-400">/ {b.target}%</span>
                {b.pct <= b.target
            ...
            <div className="relative h-3 bg-gray-100 rounded-full overflow-hidden">
              <div className="absolute h-full rounded-full transition-all duration-700"
                style={{ width: `${Math.min(b.pct, 100)}%`, backgroundColor: b.pct > b.target ? '#f87171' : b.color }} />
              <div className="absolute top-0 h-full w-0.5 bg-gray-400/30" style={{ left: `${b.target}%` }} />
            </div>
          </div>
        ))}
      </div>
      <p className="text-sm text-violet-500 mt-4">
        {bars[2].pct >= 20 ? '🎉 Bravo ! Tu respectes la règle 50/30/20.' : `💡 Épargne encore €${Math.max(0, Math.round(revenus * 0.2 - epargne))} de plus ce mois.`}
      </p>
    </div>
  );
};
...
export default function EtudiantPage() {
  ...
  const getCategoryIcon = (amount: number, category: string): string => {
    if (amount > 0) return '💰';
    const icons: Record<string, string> = { Alimentation: '🛒', Transport: '🚗', Loisirs: '🎮', Abonnements: '📱', Études: '📚', Santé: '⚕️', Prélèvement: '🏦' };
  ...
  const DashboardPage = () => (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center gap-3 p-4 bg-white rounded-2xl shadow border border-gray-100">
        ...
        <div>
          <p className="font-bold text-slate-900">Espace Étudiant</p>
          <p className="text-sm text-violet-500">Budget serré · Règle 50/30/20</p>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
        ...
        <div className="space-y-3">
          ...
          {db.transactions.length === 0 && (
            <p className="text-center text-gray-400 py-6">Aucune transaction — ajoute ta première !</p>
          )}
        </div>
      </div>
    </div>
        ...
        <div className="space-y-3">
          {db.loading ? (
            ...
            <div className="text-center py-12 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl">
              ...
              <p className="text-indigo-600">{db.searchText ? `Aucun résultat pour "${db.searchText}"` : 'Ajoute ta première transaction !'}</p>
            </div>
  ...
  const BudgetPage = () => {
    const categoryBudgets = [
      ...
      { name: 'Loisirs', budget: 80, icon: '🎮', color: '#45B7D1' },
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 text-center py-12">
        ...
        <h3 className="text-2xl font-bold mb-2">
          {db.answers?.objectifsEpargne?.length > 0 ? 'Tes objectifs' : 'Pas d\'objectifs encore'}
        </h3>
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <span className="mt-3 px-4 py-1.5 bg-indigo-50 text-indigo-700 text-sm font-bold rounded-full border border-indigo-100">🎓 Profil Étudiant</span>
        </div>
        <div className="space-y-4">
          {[
          ...
          <button onClick={db.changeProfile}
            ...
            <span className="text-sm text-violet-500">Étudiant</span>
          </button>
        </div>
      </div>
