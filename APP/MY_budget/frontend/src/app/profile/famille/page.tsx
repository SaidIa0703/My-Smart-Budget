...
const FondsUrgenceWidget = ({ stats, nbEnfants }: { stats: any; nbEnfants: number }) => {
  const cible = stats.depenses * 3;
  const epargne = Math.max(0, stats.solde);
  const pct = cible > 0 ? Math.min(Math.round((epargne / cible) * 100), 100) : 0;

  return (
    <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
      <div className="flex justify-between items-center mb-5">
        <h3 className="text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">🛡️ Fonds d'urgence familial</h3>
        <span className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full font-medium border border-indigo-100">Famille</span>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="p-3 rounded-2xl bg-gradient-to-r from-gray-50 to-transparent border border-gray-100 text-center">
          <p className="text-xs text-violet-500 mb-1">Épargne actuelle</p>
        ...
        <div className="p-3 rounded-2xl bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 text-center">
          <p className="text-xs text-violet-500 mb-1">Objectif 3 mois</p>
          <p className="font-bold text-indigo-700">€{cible.toFixed(0)}</p>
        </div>
        <div className="p-3 rounded-2xl bg-gradient-to-r from-gray-50 to-transparent border border-gray-100 text-center">
          <p className="text-xs text-violet-500 mb-1">Personnes</p>
          <p className="font-bold text-slate-900">{2 + nbEnfants} 👨‍👩‍👧</p>
        </div>
      </div>
      ...
      <p className="text-right text-xs text-violet-500 mb-3">{pct}% atteint</p>
      {nbEnfants > 0 && (
        <div className="p-3 rounded-2xl bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 mb-3">
      ...
      <p className="text-sm text-violet-500">
        {pct >= 100 ? '🎉 Fonds d\'urgence constitué ! Bravo !' : `💡 Il vous manque €${Math.max(0, cible - epargne).toFixed(0)} pour 3 mois de charges.`}
      </p>
    </div>
  );
};
  ...
  const DashboardPage = () => (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center gap-3 p-4 bg-white rounded-2xl shadow border border-gray-100">
        ...
        <div>
          <p className="font-bold text-slate-900">Espace Famille</p>
          <p className="text-sm text-violet-500">
            {nbEnfants > 0 ? `Famille de ${2 + nbEnfants} · Budget partagé` : 'Gestion multi-personnes · Dépenses partagées'}
          </p>
        </div>
        <span className="ml-auto text-xs bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full font-medium border border-indigo-100">
          {db.answers?.situationFamiliale || '—'}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-3xl p-6 text-white shadow-2xl transform transition-all duration-500 hover:scale-105">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-indigo-100 text-sm font-medium">Solde du foyer</p>
        ...
        <div className="space-y-3">
          {db.loading ? (
            ...
            <div className="text-center py-12 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl">
              ...
              <p className="text-indigo-600">{db.searchText ? `Aucun résultat pour "${db.searchText}"` : 'Ajoutez la première transaction !'}</p>
            </div>
  ...
  const BudgetPage = () => {
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Budgets du foyer</h3>
            {nbEnfants > 0 && (
              <span className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full border border-indigo-100">
                Budget Éducation × {nbEnfants} enfant{nbEnfants > 1 ? 's' : ''}
              </span>
            )}
          </div>
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 text-center py-12">
        ...
        <h3 className="text-2xl font-bold mb-2">
          {db.answers?.objectifsEpargne?.length > 0 ? 'Objectifs familiaux' : 'Pas d\'objectifs encore'}
        </h3>
        {db.answers?.objectifsEpargne?.length > 0 ? (
          ...
          <p className="text-indigo-600 mb-6">Définissez vos objectifs d'épargne familiaux</p>
        )}
        {nbEnfants > 0 && (
          <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 mb-6 text-left max-w-sm mx-auto">
            <p className="text-sm text-indigo-700">💡 Ouvrez un livret épargne pour les études de vos {nbEnfants} enfant{nbEnfants > 1 ? 's' : ''}.</p>
          </div>
        )}
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <span className="mt-3 px-4 py-1.5 bg-indigo-50 text-indigo-700 text-sm font-bold rounded-full border border-indigo-100">👨‍👩‍👧 Profil Famille</span>
          {nbEnfants > 0 && (
        ...
        <div className="space-y-4">
          ...
          <button onClick={db.changeProfile}
            ...
            <span className="text-sm text-violet-500">Famille</span>
          </button>
        </div>
      </div>
