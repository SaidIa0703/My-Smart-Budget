...
MongoDB : compteur lastLogin $<$ 7 jours + Google Analytics events & 
...
// MongoDB : Tracking des connexions
db.users.updateOne(
  { _id: userId },
  { 
...
// Requête d'analyse hebdomadaire
...
\begin{itemize}
    \item \textbf{Utilisateur final :} Consulte dashboard, saisit transactions, configure budgets → \textit{Frontend React}
    \item \textbf{Administrateur :} Gère les utilisateurs, consulte métriques globales → \textit{Backend admin}
    \item \textbf{Système bancaire :} Fournit données via API (import CSV/OFX) → \textit{Service d'import} (prévu V2)
\end{itemize}

\textbf{Architecture technique (dans le périmètre) :}
\begin{enumerate}
    \item \textbf{Frontend React} : Interface utilisateur (SPA), appels API REST via Axios
    \item \textbf{API Node.js/Express} : Logique métier, validation, authentification JWT
    \item \textbf{MongoDB} : Stockage des transactions, budgets, utilisateurs
    \item \textbf{Redis} : Cache sessions, taux de change, calculs fréquents
    \item \textbf{Service Worker} : Synchronisation offline, notifications push
\end{enumerate}
...
Chaque classe correspond à une collection dans MongoDB, avec les index appropriés pour optimiser les requêtes.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=yellow!5!white,colframe=yellow!60!black,title=\textbf{Implémentation MongoDB avec index optimisés}]
\begin{lstlisting}[language=JavaScript]
// Schéma Mongoose avec index pour performances
...
\begin{itemize}
    ...
    \item une \textbf{intégrité des données} grâce aux liens explicites entre les entités et aux validations Mongoose ;
...
\begin{itemize}
    ...
    \item Met en valeur mes compétences techniques (React, Node.js, MongoDB, Docker, AWS) avec \textbf{15k lignes de code}, \textbf{73\% de couverture de tests} et \textbf{score Lighthouse 92/100}.
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
    \item \textbf{Front-end :} création d'interfaces réactives avec \textit{React.js}, score Lighthouse Accessibility \textbf{95/100} ;
    \item \textbf{Back-end :} API REST avec \textbf{23 endpoints}, temps de réponse moyen \textbf{142ms} (P95) ;
    \item \textbf{Base de données :} \textbf{3 collections MongoDB} optimisées avec index, requêtes $<$ 50ms pour 100k docs ;
...
\begin{itemize}
    \item \textbf{Compétences techniques :} maîtrise stack MERN, \textbf{15k lignes de code}, \textbf{73\% tests coverage} ;
