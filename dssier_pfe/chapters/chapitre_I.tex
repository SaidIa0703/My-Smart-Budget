...
\section{Diagramme de contexte -- My Smart Budget}

\begin{tcolorbox}[colback=gray!5!white,colframe=gray!60!black,title=\textbf{Description du diagramme}]
Le diagramme de contexte ci-dessous positionne l'application \textbf{My Smart Budget} au sein de son écosystème.
Il représente les acteurs (Utilisateur, Administrateur, Comptable), les composants applicatifs et les services externes interfacés, ainsi que les flux d'informations entre eux.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{Interface Utilisateur (React.js)} : composant applicatif central qui orchestre les requêtes HTTP/API REST vers le backend.
    ...
    \item \textbf{Base de Données (MongoDB)} : stockage flexible des transactions et données documentaires.
...
\begin{itemize}
    \item \textbf{Flux utilisateur} : saisie/consultation via React.js $\rightarrow$ requêtes HTTP/API REST $\rightarrow$ traitement Express $\rightarrow$ stockage MongoDB/PostgreSQL ;
    \item \textbf{Flux alertes} : l'API surveille les seuils budgétaires et déclenche le Service d'Alertes (emails) en temps réel ;
    \item \textbf{Flux d'authentification} : intégration Google/Auth0 pour la vérification d'identité (OAuth2) ;
