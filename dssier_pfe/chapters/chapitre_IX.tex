...
// Migration class component → hook fonctionnel Next.js (Juin 2024)
...
\iffalse % BLOC COMPRESSE — security pipeline YAML
\begin{exemple}
...
Le pipeline exécute quotidiennement : \textbf{npm audit}, \textbf{Snyk} (CVE), \textbf{OWASP ZAP} (DAST) et \textbf{Trivy} (images Docker). Tout résultat critique bloque le merge.
...
\begin{exemple}
\textbf{3 améliorations concrètes issues de la veille :}
\begin{lstlisting}[language=JavaScript]
// 1. bcrypt → Argon2id (Oct 2025) : résistance GPU/ASIC +400%
const hashedPwd = await argon2.hash(password,
  { type: argon2.argon2id, memoryCost: 2**16, timeCost: 3 });

// 2. CSP avec nonces (Nov 2025) : score A+ Observatory, 0 XSS
res.locals.nonce = crypto.randomBytes(16).toString('base64');
helmet.contentSecurityPolicy({ directives: {
  scriptSrc: ["'self'", (req, res) => `'nonce-${res.locals.nonce}'`]
}});

// 3. Rate limiting adaptatif par endpoint (Déc 2025) : -94% brute force
