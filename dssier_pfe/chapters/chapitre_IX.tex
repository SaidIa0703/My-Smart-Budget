...
\hline
Node.js & 18.19.0 & 21.5.0 & 20.10.0 & Migration vers 20 LTS planifiée Q1 2026 (Dockerfile actuel : node:18-alpine) \\
\hline
...
\hline
Docker & 24.0.7 & 25.0.0 & 24.0.x & BuildKit par défaut, testé en local avant prod \\
\hline
...
\begin{itemize}
    ...
    \item \textbf{Décision :} POC réalisé, migration planifiée en v2.0 (bcrypt actuellement en production)
\end{itemize}
\end{tcolorbox}
...
\begin{exemple}
\textbf{Pipeline de sécurité (objectif v2.0 -- actuellement intégré dans sonarcloud.yml) :}
\begin{lstlisting}[language=yaml]
# .github/workflows/security.yml  (planifie v2.0)
name: Security Scanning Pipeline

on:
  schedule:
    - cron: '0 0 * * *'  # Quotidien a minuit
  push:
    branches: [main]
  pull_request:
...
\begin{lstlisting}[language=JavaScript]
// 1. Bcrypt en production (Cost 10) -- migration Argon2id planifiée v2.0
const bcrypt = require('bcryptjs');
const hashedPassword = await bcrypt.hash(password, 10);
// Veille : Argon2id identifié comme successor recommandé (OWASP)
// Action : POC réalisé, migration planifiée pour v2.0
...
\subsection{Roadmap technologique 2025--2026}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!60!black,title=\textbf{Plan de migration et évolutions planifiées}]
\textbf{Q1--Q2 2025 - Modernisation Core (appliqué) :}
\begin{itemize}
    \item 📋 Node.js 20 LTS : migration planifiée Q1 2026 (actuellement 18.19.0)
    \item ✅ Next.js 15 GA : Turbopack stable, Server Actions
    \item ✅ PostgreSQL 16 : Meilleures performances parallélisme
    \item ✅ TypeScript 5.4 : Nouveaux types, performances accrues
    \item ✅ GitHub Copilot : Intégration développement
\end{itemize}

\textbf{Q3--Q4 2025 - Sécurité \& Performance (appliqué) :}
\begin{itemize}
    \item ✅ Renforcement CSP stricte avec nonces : score A+ Mozilla Observatory
    \item ✅ Index composites PostgreSQL : -56\% temps requête dashboard
    \item ✅ Compression gzip Nginx : -70\% taille payload
    \item 🔬 Bun runtime : benchmarks réalisés (+67\% tests), adoption planifiée 2026
\end{itemize}

\textbf{2026 - Évolutions planifiées :}
\begin{itemize}
    \item 📋 Migration Node.js 22 LTS (Q1 2026)
    \item 📋 React Server Components : migration progressive Dashboard (Q2 2026)
    \item 📋 POC catégorisation automatique ML (Q3 2026)
    \item 📋 Évaluation PostgreSQL 17 JSON vs MongoDB pour données semi-structurées
\end{itemize}
\end{tcolorbox}
...
\hline
Veille Argon2id vs bcrypt & POC réalisé, migration planifiée v2.0 & OWASP Password Storage \\
\hline
...
└── 📝 ADR (Architecture Decision Records)
    ...
    ├── ADR-002: Choix bcrypt (Cost 10) -- evaluation Argon2id planifiee v2.0
    ├── ADR-003: Adoption TypeScript
    └── ADR-004: Choix Express vs NestJS -- légèreté et contrôle total
\end{verbatim}
\end{exemple}
...
\begin{itemize}
    ...
    \item \textbf{ADR-002 :} Choix bcrypt (Cost 10) pour le hachage -- évaluation Argon2id en cours (v2.0)
...
\begin{itemize}
    ...
    \item ✅ \textbf{-58\% bundle size} grâce aux optimisations Next.js
    \item ✅ \textbf{100\% mises à jour} appliquées sans incident de régression
...
\textbf{Objectifs 2026 :}
\begin{itemize}
    \item Maintenir 0 vulnérabilité critique
    \item Atteindre 90\% de couverture de tests (actuellement 85,3\%)
    \item Réduire le Time to Market de 20\% via optimisation pipeline CI/CD
    \item Déployer le POC de catégorisation automatique (ML)
    \item Migrer vers Node.js 22 LTS
\end{itemize}
\end{tcolorbox}
