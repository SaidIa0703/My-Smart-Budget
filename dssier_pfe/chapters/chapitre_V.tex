...
\begin{itemize}
    \item \textbf{Tier 1 (Client) :} React 18 + TypeScript + Redux Toolkit.
...
\begin{center}
\small$\rightarrow$~\textit{Flux de données 3-tiers — voir Annexe~\ref{ann:figures}, figure~\ref{fig:arch_impl}}
\end{center}
...
|   +-- useTransactions.ts
|   +-- useBudgetAlerts.ts
+-- store/                 # Gestion d'état global (Redux)
...
const TransactionForm = ({ onSubmit, categories, budget }) => {
  ...
  return (
    <form onSubmit={handleSubmit} aria-label="Ajouter une transaction">
      {/* Champs de formulaire Material-UI avec validation */}
      <TextField
        label="Montant"
        type="number"
...
\begin{conseil}
\textbf{Bonnes pratiques Frontend appliquées :}
\begin{itemize}
    \item Utilisation de \textbf{React Hook Form} pour la gestion performante des formulaires.
    \item Typage strict avec \textbf{TypeScript} (Interfaces pour Props et State).
    \item Séparation de la logique (Hooks) et de la vue (Components).
\end{itemize}
\end{conseil}
...
\begin{jury}
\textbf{Questions potentielles lors de la soutenance :}
\begin{itemize}
    \item \textit{"Pourquoi avoir mélangé PostgreSQL et MongoDB ?"} \\
    $\rightarrow$ Réponse : PostgreSQL garantit la cohérence des transactions financières (ACID), tandis que MongoDB offre la flexibilité et la rapidité d'écriture pour les logs et les données analytiques non structurées.
    \item \textit{"Comment gérez-vous les erreurs d'arrondi sur les montants ?"} \\
    $\rightarrow$ Réponse : Utilisation du type \texttt{DECIMAL} en base de données et manipulation des montants en centimes (entiers) côté serveur avant conversion.
\end{itemize}
\end{jury}
...
En résumé, cette couche d'implémentation traduit fidèlement les choix de conception en code fonctionnel, testé et sécurisé. Ce que le jury doit retenir : chaque couche a une responsabilité claire, et aucune logique métier ne s'est glissée dans les contrôleurs ou les composants React.