...
\hline
\textbf{A02} - Cryptographic Failures & Élevé & TLS 1.3 obligatoire, Bcrypt (Cost 12) pour les mots de passe, Chiffrement AES-256 des données sensibles. \\
\hline
\textbf{A03} - Injection & Élevé & Utilisation de l'ORM Prisma (requêtes paramétrées) et validation Joi/Zod en entrée. \\
\hline
...
module.exports = (app) => {
  ...
  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc: ["'self'"],
        scriptSrc: ["'self'", "'unsafe-inline'"], // Adapte pour React
...
async deleteAccount(req, res) {
  ...
  try {
    ...
    await User.findByIdAndUpdate(userId, {
      email: `deleted-${uuid()}@anonyme.local`,
      firstName: 'Deleted',
      lastName: 'User',
      password: null,
      isDeleted: true,
...
\subsection{Résultats des scans (Q4 2024)}
...
\begin{itemize}
    ...
    \item XSS Stored : \textcolor{green!60!black}{\textbf{Échec}} (Échappement React + CSP)
...
\begin{itemize}
    \item \textbf{MFA (Q1 2025) :} Ajout de l'authentification à double facteur (TOTP).
...
Ce chapitre démontre que l'application \textbf{My Smart Budget} respecte les standards de l'industrie, garantissant confidentialité, intégrité et disponibilité des données utilisateurs.