...
describe('Transactions API Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    testUser = await db.one(
      ...
      ['test@mysmartbudget.com', await bcrypt.hash('Test123!@#', 10),
       'Test', 'User']
    );
...
\begin{tcolorbox}[colback=green!5!white,colframe=green!60!black,title=\textbf{Couverture de code actuelle (Février 2026)}]
\begin{center}
...
\begin{tcolorbox}[colback=green!5!white,colframe=green!60!black,title=\textbf{Dashboard SonarQube - My Smart Budget (15 février 2026)}]
\begin{center}
\begin{figure}[H]
    \centering
...
\begin{itemize}
    ...
    \item ✅ \textbf{Pipeline CI/CD robuste :} 92.9\% de succès
